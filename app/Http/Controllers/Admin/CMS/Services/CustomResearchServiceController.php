...
use App\Models\Arm_service_custom_research;
use App\Traits\MediaTrait;
use App\Models\Arm_role_privilege;
use Auth;

class CustomResearchServiceController extends Controller
{
    use MediaTrait;
    ...
    public function index(){
        ...
        if (!empty($RolesPrivileges) && str_contains($RolesPrivileges, 'cms_view') && str_contains($RolesPrivileges, 'page_content_view') && str_contains($RolesPrivileges, 'custom_research_service_view')){
            $service=Arm_service_custom_research::where('status','=','active')->first();
            return view('admin.cms.pages_content.services_custom_market_research',compact('service'));
        }else {
        ...
        if ($request->section_id == '1') {
            ...
            if (!empty($id)) {
                if (!empty($RolesPrivileges) && str_contains($RolesPrivileges, 'custom_research_service_edit')){
                    ...
                    Arm_service_custom_research::where('id', '=', $id)->update($input);
                    return redirect('/admin/services-custom-market-research')->with('success', 'Updated Successfully!');
                }else {
                    return redirect()->back()->with('error', 'Sorry, You Have No Permission For This Request!');
                }
            } else {
                if (!empty($RolesPrivileges) && str_contains($RolesPrivileges, 'custom_research_service_add')){
                    $input['created_by'] = auth()->guard('arm_admins')->user()->id;
                    $input['created_ip_address'] = $request->ip();
                    Arm_service_custom_research::create($input);
                    return redirect('/admin/services-custom-market-research')->with('success','Added Successfully!');
                }else {
                    return redirect()->back()->with('error', 'Sorry, You Have No Permission For This Request!');
                }
            }
    ...
    public function show(){
       $service = Arm_service_custom_research::where('status', '=', 'active')->first();
        return view('front.custom-research-services', compact('service')); 
    }
}

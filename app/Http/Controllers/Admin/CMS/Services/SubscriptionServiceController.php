...
use App\Models\Arm_service_subscription;
...
class SubscriptionServiceController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($RolesPrivileges) && str_contains($RolesPrivileges, 'cms_view') && str_contains($RolesPrivileges, 'page_content_view') && str_contains($RolesPrivileges, 'service_subscription_view')){
            $service = Arm_service_subscription::where('status', '=', 'active')->first();
            return view('admin.cms.pages_content.services_subscription',compact('service'));
        }else {
        ...
        if ($request->section_id == '1') {
            ...
            if (!empty($id)) {
                if (!empty($RolesPrivileges) && str_contains($RolesPrivileges, 'service_subscription_edit')){
                    ...
                    Arm_service_subscription::where('id', '=', $id)->update($input);
                    return redirect('/admin/services-subscription')->with('success', 'Updated Successfully!');
                }else {
                    return redirect()->back()->with('error', 'Sorry, You Have No Permission For This Request!');
                }
            } else {
                if (!empty($RolesPrivileges) && str_contains($RolesPrivileges, 'service_subscription_add')){
                    $input['created_by'] = auth()->guard('arm_admins')->user()->id;
                    $input['created_ip_address'] = $request->ip();
                    Arm_service_subscription::create($input);
                    return redirect('/admin/services-subscription')->with('success', 'Added Successfully!');
                }else {
                    return redirect()->back()->with('error', 'Sorry, You Have No Permission For This Request!');
                }
            }
    ...
    {
        $service = Arm_service_subscription::where('status', '=', 'active')->first();
        return view('front.subscription', compact('service'));
    }
}
